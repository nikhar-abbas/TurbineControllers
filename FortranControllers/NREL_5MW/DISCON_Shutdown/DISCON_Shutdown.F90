...
SUBROUTINE DISCON ( avrSWAP, aviFAIL, accINFILE, avcOUTNAME, avcMSG ) BIND (C, NAME='DISCON')
   ...
   ! Modified by Nikhar Abbas to include Shutdown supervisory control (April 2019).
...
REAL(4), PARAMETER           :: PC_MaxPit     =       2 * atan(1.0_8) !1.570796                  ! Maximum pitch setting in pitch controller, rad.
...
REAL(4), SAVE                :: VS_TrGnSp                                       ! Transitional generator speed (HSS side) between regions 2 and 2 1/2, rad/s.
   ! Shutdown
Real(4), PARAMETER           :: SH_MaxPit    = 0.2617                           ! Maximum blade pitch before shutdown, rad.
Real(4), PARAMETER           :: SH_Tau       = 10                               ! Time constant of the first order low pass filter for the blade pitch control signal, s. 
Real(4), SAVE                :: BlPitchF                                        ! Filtered blade pitch angle for shutdown supervisory controller
LOGICAL(1), SAVE             :: SD                                              ! Logical to enable shutdown controller
REAL(4)                      :: AlphaBld                                        ! Current coefficient in the recursive, single-pole, low-pass filter for blade pitch angle (-).
REAL(4)                      :: PitCom_SH                                       ! Total command pitch based on the shutdown routine, rad.
...
IF ( iStatus == 0 )  THEN  ! .TRUE. if we're on the first call to the DLL
   ...
   BlPitchF  = BlPitch(1)                       ! This will ensure that the blade pitch filter will use the initial value of the blade pitch on the first pass
   SD = .FALSE.                                 ! This will ensure that the shutdown controller is off intitially.
   PitCom_SH = SH_MaxPit                       ! This will ensure that the shutdown controller begins pitching at the defined shutoff blade pitch angle
ENDIF
...
!=======================================================================
   ...
   ! Shutdown - don't allow the turbine to motor
      IF (SD) THEN
         GenTrq = MIN( MAX( GenTrq, 0.0), VS_MaxTq)
      ENDIF
...
! ======== SHUTDOWN TESTING =========
   ! Blade Pitch Filter
   AlphaBld     = EXP( ( LastTime - Time )*1/SH_Tau )
   ! Apply the filter:
   BlPitchF = ( 1.0 - AlphaBld )*BlPitch(1) + AlphaBld*BlPitchF

   ! Pitch Blades to 90 degrees if in shutdown mode
   IF ( (SD)  .OR. (BlPitchF > SH_MaxPit) ) THEN
      print*, 'Shutting down.....'
      print*, 'Current Blade Pitch:', BlPitch(1)
      PitCom_SH = PitCom(1) + PC_MaxRat*ElapTime
      DO K = 1,NumBl
         PitCom(K) = MIN( MAX( PitCom_SH, PC_MinPit ), PC_MaxPit )
      ENDDO
      print *, 'Saturated shutdown pitch angle:', PitCom(1)
